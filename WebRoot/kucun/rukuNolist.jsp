...
<html>
  ...
  <div style="" align='center'>
    <input id="rukuNoSearch" type="text"  value="请输入查询内容" onclick="JavaScript:this.value=''" style="height:25px;;margin-top:20px;"/>
  ...
  <table class="layui-table" id="demo" lay-filter="test">
	...
	<tbody>
         <c:forEach items="${rukuNolist}" var="inout">
				<tr>				  
					<td>${inout.getId() }</td>
					<td>
					...
					<td >
					     ...
					     <a href="javascript:rukuSuccess('${inout.getId() }')">入库</a>
					     <a href="torukuApplyPrinting.do?id=${inout.getId() }">入库表单打印</a>
					</td>
				</tr>
		 </c:forEach>      
	</tbody>
  ...
  function rukuSuccess(id){
		if(window.confirm("确认产品入库？")){
			window.location.href = "rukuSuccess.do?id="+id;
		}
	}
  function search(){
		var rukuNoSearch=document.getElementById("rukuNoSearch").value;	
		if(rukuNoSearch=="请输入查询内容" || rukuNoSearch=="")
			window.alert("请输入查询内容");
		else	
	        window.location.href = "rukuNoSearch.do?rukuNoSearch="+rukuNoSearch;		  	
   }
  </script>
  </body>
